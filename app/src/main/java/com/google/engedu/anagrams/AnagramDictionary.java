...
public class AnagramDictionary {
    ...
    public AnagramDictionary(Reader reader) throws IOException {
        ...
        for (int count = 0; count < dictionary.size(); count ++){
            Integer key = dictionary.get(count).length();   //get length of current word in the dictionary
            ArrayList<String> sizeList = new ArrayList<String>();   //list in the value sections of the hash map
            if(!sizeToWords.containsKey(key)){
                sizeList.add(dictionary.get(count));     //add word
                sizeToWords.put(key, sizeList);         //update
            }
            else {
                sizeToWords.get(key).add(dictionary.get(count));    //just add to the value list at the key
    ...
    //pick word based on length and number of possible anagrams
    public String pickGoodStarterWord() {
        ...
        //pick random start point in wordList and find one with min numbers of anagrams
            ...
            //as long as length is less than the MAX, increase letter length
